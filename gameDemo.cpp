...
*/
		...
		bool actionProcessing(Environment, Character, string, int &, int &, bool &) ;	//prompts situation, processes user action, 
...
{//~~~~Load the Neccesary Files~~~~~~ and validate	     ******************FileI/O Demonstrated HERE********************
			environmentsFile.open("environments.txt") ;	//opening the file for saved environments
...
{//~~~~The Game~~~~~ 
		//~~~~Prompt, take in action, process action, rinse repeat~~~~
				int currentPlace = 0, lastPlace = 1 ;	//default entrance environment
				bool moved = true ;
			while(true)		//~~~working ~~~~~prints environment description, processes action input
			{
//				bool repeat = false ;
				string action = environments[currentPlace].situationAndAction(moved) ;	//class function prompts environment descript and return user action string
				moved = false ;
				while(true)
				{	
					if (!actionProcessing(environments[currentPlace], characters[playerIndex], action, currentPlace, lastPlace, moved))		//don't move on, without a valid argument from user for environment action list 
					{
						cout << "huh?" << endl ;
						break ;
					}
		...
		bool actionProcessing(Environment place, Character player, string action, int &move, int&last, bool &moved)		//more work to do add functionality and actions
		{								//Game Wide Commands
			if (action == "back")
			{
				int temp = last ;
		...
		//	cout << "Description ---> " << description << endl ;
			for (int i = 0; i < NUMBEROFENVIRONMENTS ; i++) 
			{
				...
				getline(environmentsFile, exit) ;
				//cout << "The exit being logged is " << exit << endl ;
				if (exit == "end")
				{
					break ;
				}
				...
				{
					for (int i = 0 ; i < exit.length() ; i++)		//set user input toLower case
					{
						if (exit[i] > 64 && exit[i] < 91)
						{
							exit[i] = exit[i] + 32 ;
						}
					}
		...
		//			cout << "ExitNumber ---> " << exitNum << endl ;
			...
			for (int i = 0; i < NUMBEROFENVIRONMENTS ; i++) 
			{
				...
				getline(environmentsFile, objectDescription) ;
					for (int i = 0 ; i < object.length() ; i++)		//set user input toLower case
					{
						if (object[i] > 64 && object[i] < 91)
						{
							object[i] = object[i] + 32 ;
						}
					}
				objects[i] = object ;
